 ...
 * 上传到upyun
 * @param opts
 * @param opts.signUrl 获取签名的url
 * @param opts.cdnSpace 文件空间名
 * @param opts.fileHash 文件md5
 * @param opts.uploadFn 上传成功后的方法
 * @param opts.uploadingFn 上传中的方法
 * @param opts.errorFn 上传报错时的调用 方法
 * @param opts.abortFn 上传中断时的调用方法
 */
function upyun(opts) {
    var data = new FormData();
    data.append('type', opts.cdnSpace);
    data.append('hash', opts.fileHash);
    var ar = opts.file.name.split('.');
    data.append('ext', ar[ar.length - 1]);

    ajax({
        url: opts.signUrl, form: data, success: function (data) {
            //var data = JSON.parse(xhr.responseText);
            if (data.url && data.api) {
                var fd = new FormData();
                fd.append('policy', data.post.policy);
                ...
                xhrUpyun.addEventListener("load", function () {
                    ...
                    if (dataUpyun.message && dataUpyun.message == 'ok') {
                        ...
                        typeof opts.cdnErrorFun === 'function' ? opts.cdnErrorFun(dataUpyun) : console.log('cdn错误');
                    }
                }, false);
                ...
                xhrUpyun.addEventListener("error", function (evt) {
                    (typeof opts.errorFn === 'function' ) ? opts.uploadingFn(evt) : console.log(img, '出错了');
                }, false);
                xhrUpyun.addEventListener("abort", function () {
                    (typeof opts.abortFn === 'function' ) ? opts.abortFn(evt) : console.log(img, '中断了');
                }, false);
                console.log(data.api);
                xhrUpyun.open("POST", data.api);
                xhrUpyun.send(fd);
